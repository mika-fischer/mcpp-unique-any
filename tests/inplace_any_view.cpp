...
static_assert(sizeof(inplace_any_view) >= 3 * sizeof(void *));

} // namespace

TEST_CASE("basic") {
    alignas(alignof(large)) std::array<std::byte, sizeof(large)> buffer;
    auto any = inplace_any_view(buffer.data(), buffer.size());
    CHECK(!any.has_value());
...
TEST_CASE("small_buffer") {
    alignas(alignof(small)) std::array<std::byte, sizeof(small)> buffer;
    auto pre = n_allocs;
    auto any = inplace_any_view(buffer.data(), buffer.size());
    any = small{};
...
TEST_CASE("throwing") {
    alignas(1) std::array<std::byte, 1> buffer;
    auto any = inplace_any_view(buffer.data(), buffer.size());
    CHECK_THROWS_MESSAGE(any = large{}, "Type is too large for inplace_any_view buffer.");
}
